...
/**
 * ScannerFragment class scan required BLE devices and shows them in a list. This class scans and
 * filters devices with standard or custom BLE Service UUID. It contains a list and a button to
 * scan/cancel. There is an interface {@link OnDeviceSelectedListener} which is implemented by
 * activity in order to receive selected device. The scanning will continue for 5 seconds.
 */
public class ScannerFragment extends DialogFragment {
    ...
    private UUID mUuid;
    private BluetoothDevice m_deviceTarget;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(final AdapterView<?> parent, final View view, final int position, final long id) {
                stopScan();
                dialog.cancel();
                // tell the activity a device select
     ...
     * then mLEScanCallback is activated This will perform regular scan for custom BLE Service UUID and then filter out
    ...
    private void startScan() {
        ...
        Log.d(TAG, "mUuid = " + mUuid + " mIsCustomUUID = " + mIsCustomUUID);
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (mIsScanning) {
                    Log.d("ldsdebug", "scanning ending");
                    stopScan();
                    mListener.onDeviceSelected(m_deviceTarget);
                    onDestroyView();
                }
            }
        }, SCAN_DURATION);
    ...
    private void addBondedDevices() {
        ...
        for (BluetoothDevice device : devices) {
            ...
            if(device.getName().equals("AURORA-BLE-RCU") || device.getName().equals("ADVRF-BLE-RCU") || device.getName().equals("ADVRF-RCUM")) {
                Log.d("ldsdebug", device.getName());
                m_deviceTarget = device;
            }
        }
    }
